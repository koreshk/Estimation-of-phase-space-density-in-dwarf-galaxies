...
###################################################################
def sigma_DM_vs(r2,nu,M,Mcentral,beta,betaf,nupars,Mpars,\
            betpars,\
            Mstar_rad,Mstar_prof,Mstar,Arot,Rhalf,G,rmin,rmax):
    #Calculate projected velocity dispersion profiles
    #given input *functions* nu(r); M(r); beta(r); betaf(r).
...
def betaf_DM(r, betpars, Rhalf, Arot):
    bet0 = betpars[0]
    betinf = betpars[1]
    r0 = betpars[2]
    n = betpars[3]
...
def binthedata(R,ms,Nbin):
    ...
    if (Nbin == -1):
        ...
        rbin = np.zeros(len(R))
        rbin[0] = R[0]/2
        rbin[1:] = np.array([R[i-1] + (R[i]-R[i-1])/2 for i in range (1,len(R))])
        Nbin = np.zeros(len(ms))
        Nbin[0] = ms[0]*R[0]**2*np.pi
        Nbin[1:] = ms[1:]*(R[1:]**2-R[:-1]**2)*np.pi
